...
○ 캐릭터 셋(Character Set)
   ...
   -유니코드(unicode)
	...
	한글이나 일본어는 16비트의 값을 가지게 되는데 이들을 모두 16비트로 통일한 것이다.

   -UTF-8과 UTF-16
    UTF-8과 UTF-16의 차이를 한 마디로 말하면
	문자로 표현할 때의 단위가 다르다는 것이다.
	UTF-8은 8비트이 가변장 멀티바이트에서 문자를 표현하고
...
public class Test136
{
	public static void main(String[] args) throws UnsupportedEncodingException
	{
		...
		try
		{
			...
			System.out.println("value1: "+new String(buf, "UTF-8"));	//--vale1: 장서현
			...
			System.out.println("value2: "+new String(buf));	//--value2: ?옣?꽌?쁽
			...
			System.out.println("value3: "+convert);	//--value3: ????

			buf = convert.getBytes();
			System.out.println("buf 배열의 length: "+buf.length);	//--buf 배열의 length: 4

			buf = convert.getBytes("UTF-8");
			System.out.println("buf 배열의 length: "+buf.length);	//--buf 배열의 length: 12
